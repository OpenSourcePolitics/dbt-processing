    WITH participations_proposals AS (
    ...
    ), participations_initiatives as (
            SELECT decidim_users.id,
                0 as decidim_component_id,
                'Decidim::Initiatives::Initiative' as "participation_type",
                decidim_initiatives.id::text as "participation_id", 
                decidim_initiatives.created_at as participation_date
            FROM {{ ref("initiatives")}} as decidim_initiatives
                JOIN {{ ref("users")}} AS decidim_users on decidim_users.id = decidim_initiatives.decidim_author_id
    ), participations as (
        ...
        SELECT * from participations_meetings_registrations union all
        SELECT * from participations_initiatives
    )
    SELECT
